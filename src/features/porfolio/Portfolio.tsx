...
export function Portfolio(props: Props): JSX.Element {
  ...
  const getMessage = (negativeArray: Diff[], positiveArray: Diff[]) => {
    ...
    while (i < positiveArray.length && j < negativeArray.length) {
      let sum = Math.round((currPosVal + currNegVal) * 100) / 100;
      if (sum > 0) {
        message += `• Transfer ${Math.abs(currNegVal)} from ${negativeArray[j].key} to ${positiveArray[i].key} \n`;
        j++;
  ...
  const rebalance = () => {
    ...
    if (
      isNaN(_bonds) || isNaN(_largeCap) || isNaN(_midCap) || isNaN(_foreign) || isNaN(_smallCap)
    ) {
      setMessage([
        false,
    ...
    const diffB = Math.round((total * riskSelected.Bonds / 100 - Number(bonds)) * 100) / 100;
    const diffL = Math.round((total * riskSelected["Large Cap"] / 100 - Number(largeCap)) * 100) / 100;
    const diffM = Math.round((total * riskSelected["Mid Caps"] / 100 - Number(midCap)) * 100) / 100;
    const diffF = Math.round((total * riskSelected.Foreign / 100 - Number(foreign)) * 100) / 100;
    const diffS = Math.round((total * riskSelected["Small Cap"] / 100 - Number(smallCap)) * 100) / 100;
    ...
    let negativeArray: Diff[] = [];
    let positiveArray: Diff[] = [];
    [
          ...
          <Grid className={style.riskCalculatorContainer}>
            ...
            <Cell small={4} className={style.riskCalculatorTransfer}>
              <div className={style.riskCalculatorTransferWrap}>
                <p className={`${message[0] ? style.red : ""}`}>
                  {message[1]}
                </p>
              </div>
            </Cell>
